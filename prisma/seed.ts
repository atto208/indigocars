...
// Indigo Cars' real fleet. Website Car rows = one per model (deduped);
// FleetCar = the physical cars for the calendar. Idempotent: skips non-empty
// tables. To wipe & reload instead, run scripts/reset-real-fleet.mjs.
//
// Studio car images (public/cars/*.jpg) are AI studio renders generated with
// Higgsfield's marketing_studio_image model (silver, front 3/4, on white).

// [name, brand, category, file, seats, hp, transmission, fuel, luggage, year, featured, imaginMake, imaginModel]
const cars = [
...
// Physical fleet for the calendar — the owner's real list, year in the name.
const fleet = [
  "Renault Megane — 2023",
  "Fiat Egea #1 — 2022",
  ...
  if ((await prisma.car.count()) === 0) {
    ...
    for (const [name, brand, category, file, seats, hp, transmission, fuel, luggage, year, featured, im, imm] of cars) {
      await prisma.car.create({
        data: {
          name, brand, category, image: `/cars/${file}.jpg`,
